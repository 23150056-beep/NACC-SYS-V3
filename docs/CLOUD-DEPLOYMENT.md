...
[`render.yaml`](../render.yaml) at the repo root declares the database, the API
...
3. Fill in the values Render marks as required (these are the `sync: false`
   ...
   database automatically. `RENDER_EXTERNAL_HOSTNAME` is injected at runtime and
   the app appends it to `ALLOWED_HOSTS` and `CSRF_TRUSTED_ORIGINS` itself.
...
**Database.** Enable the platform's automated backups (daily, ≥ 7 days
