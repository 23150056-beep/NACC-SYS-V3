...
import { Card, Button, Badge, Input, Select, FormField, Switch, Alert, Icon, PAGE } from '../ui';
...
export default function Settings() {
  ...
  const saveConfig = async () => {
    try {
      if (ai) {
        // Both are server-computed and read-only; sending them back is noise.
        const { updated_at, hosted_key_configured, ...payload } = ai;
  ...
  return (
    <div style={{ ...PAGE, maxWidth: 760 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <Card eyebrow="AI Assistance" title="AI Layer" padding="22px">
          {!ai ? (
            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Loading…</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {ai.provider === 'hosted' ? (
                <Alert disclaimer title="Processed by a contracted provider.">
                  Drafts are generated by a hosted model, so case text leaves this server for
                  processing. The provider must be covered by the agency's data-processing
                  agreement under RA 10173. Every output is a draft the psychologist confirms;
                  the system is fully functional with AI off.
                </Alert>
              ) : (
              ...
              <FormField
                ...
                hint="On-premises keeps every draft on this machine. Hosted is for cloud deployments with no GPU host of their own."
              >
                <Select value={ai.provider} onChange={(e) => setAi({ ...ai, provider: e.target.value })} disabled={!ai.enabled}>
                  <option value="ollama">On-premises (Ollama)</option>
                  <option value="hosted">Hosted API (cloud)</option>
                </Select>
              </FormField>

              {ai.provider === 'hosted' ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {/* The credential lives in the server environment and is never
                      editable here — surface only whether one is present, so an
                      admin isn't left guessing why every draft returns 503. */}
                  {!ai.hosted_key_configured && (
                    <Alert tone="warning" title="No API key on the server.">
                      Set <code>AI_HOSTED_API_KEY</code> in the server environment and redeploy.
                      Until then, AI features will report as unavailable.
                    </Alert>
                  )}
                  <FormField label="Model" hint="Configured by the server administrator; the API key is never stored in this system.">
                    <Input value={ai.hosted_model} onChange={(e) => setAi({ ...ai, hosted_model: e.target.value })} disabled={!ai.enabled} placeholder="claude-opus-5" />
                  </FormField>
                </div>
              ) : (
